...
#include "llvm/Transforms/frame-pass/SolitaryFramer.h"

#define DEBUG_TYPE "solitaryframer"
...
STATISTIC (solitaryframerpasscounter, "SolitaryFramer");
...
static RegisterPass<SolitaryFramer> 
X ("solitaryframer", "Solitary Framer Pass", false, true);

char SolitaryFramer::ID = 0;

// deleted for LTO
/*
static void registerFramer(const PassManagerBuilder &,
...
void 
SolitaryFramer::insertCastInstForArg (Instruction * insertBeforeMe, 
...
GlobalVariable* 
SolitaryFramer::getNextGV (GlobalVariable * GV, 
                           Module &M)
{
...
Value * 
SolitaryFramer::createConstantIntInstance (unsigned val, 
                                          Type * destTy)
{
...
Value * 
SolitaryFramer::constructValueFromFunc (unsigned val, 
                                        Function * F, 
                                        unsigned paramIdx)
{
...
Value * 
SolitaryFramer::getArraySize (AllocaInst * AI, 
                              Type * funcParamTy) 
{
...
Type * 
SolitaryFramer::getTypeOfElemOfArray (Type * allocatedType)
{
...
void 
SolitaryFramer::insertCastToRestoreType (Instruction* insertAfterMe, 
...
Instruction* 
SolitaryFramer::doArrayAllocaInst (AllocaInst * AI, 
                                   ...
                                   Module &M)
{
    ...
    unsigned numBytes   =  SolitaryFramer::getBitwidth(typeOfElem)/8;
    ...
    if (notDynamicArray){ 
        ...
        if ((uintptr_t)(SolitaryFramer::getBitwidth(AI->getAllocatedType())) >= headerTySize) {
...
/* not used 
Value * SolitaryFramer::getArrayEndAddress 
...
*/
/*
unsigned SolitaryFramer::getPrimTyBits (unsigned Typeid)
{
    unsigned bitnum;
...
*/
/*
uint64_t SolitaryFramer::getBitwidthOfStruct ( StructType * STy )
{
    if (!STy->isSized()) {
...
unsigned SolitaryFramer::getBitwidth (Type * allocatedType)
{
    ...
    if (( allocatedTyID > Type::VoidTyID && allocatedTyID < Type::PPC_FP128TyID) ){
        ...
        if (isa<PointerType>(allocatedType)) {
            ...
            return (unsigned)(arrayTy->getNumElements())*SolitaryFramer::getBitwidth(arrayTy->getElementType()); 
        }
            ...
            if (vecty->getBitWidth()!=0) {
                ...
                return (unsigned)(vecty->getNumElements())*SolitaryFramer::getBitwidth(vecty->getElementType()); 
            }
        }
        else {
...
bool SolitaryFramer::isItsUser (Value * useuser, User * useruser)
{
...
Instruction* SolitaryFramer::castAndreplaceAllUsesWith(Value * originalPtr, 
                                               Value* modifiedPtr)
{
...
int SolitaryFramer::insertLiteralToStructList (StructType* ty)
{
    allStructTyList.push_back(ty);
    return (int)(allStructTyList.size()-1);
}

int SolitaryFramer::getUserStructTypeID (StructType* ty)
{
    int structtyid= -1; 
...
int SolitaryFramer::getUserIntegerTypeID (IntegerType* ty)
{
    unsigned intID= -1;
...
unsigned 
SolitaryFramer::getFramerTypeID (Type* ty)
{
...
Constant* SolitaryFramer::constructAddressValue (Module &M, GlobalVariable* tab, unsigned subindex)
{
    Type* ty= tab->getType(); 
...
bool SolitaryFramer::isFramerIntID (unsigned id)
{
    bool isInt;
                  ...
                  Module & M)
{
    ...
    else if (isa<SequentialType>(pt)) {
        elemNum=(unsigned)cast<SequentialType>(pt)->getNumElements();  
    }
    else {
        errs()<<"other type: "<<*pt<<"\n";
        errs()<<"gep: "<<*gep<<"\n";
...
/*
bool SolitaryFramer::isFramerStructID (unsigned id)
{
    bool isStruct;
...
/*
Constant* SolitaryFramer::getFramerTyEntryAddress (Module &M, unsigned ftid)
{
...
Instruction* 
SolitaryFramer::doPrimitiveAllocaInst(AllocaInst * AI, 
                              Instruction * successorInst, 
                              vector <AllocaInst*> & LocalUnions, 
                              Module &M)
{
    ...
    unsigned numBytes  = (SolitaryFramer::getBitwidth(AI->getAllocatedType()))/8;
...
Instruction* 
SolitaryFramer::doAllocaInst (AllocaInst* AI, 
                              Instruction* successorInst,
                              //vector <GetElementPtrInst*> & LocalUnions, 
                              vector <AllocaInst*> & LocalUnions, 
                              Module & M) //,
...
Value *  SolitaryFramer::getOriginalAlloca (Value * ptr) 
{
    StringRef hookName = "";
...
/* not used
void SolitaryFramer::restoreModifiedPtr (LoadInst * LI) 
{
...
void SolitaryFramer::castAndreplaceUsesOfWithBeforeInst(Instruction* user, 
                                                Value* from, 
                                                Value * toThis)
{
...
Value* SolitaryFramer::getAssignment(LoadInst * LI, DominatorTree & dt)
{
...
bool 
SolitaryFramer::checkSafeWithDomTree (Value * op, DominatorTree & dt) 
{
...
/*bool SolitaryFramer::isStaticInBound (unsigned offset, unsigned sizeToAccess, unsigned totalsize)
{
...
//unsigned SolitaryFramer::getTotalSize(GEPOperator * gep)
/*
static unsigned 
...
*/
bool SolitaryFramer::isuntaggedPtr(Value * ptrop)
{
...
void
SolitaryFramer::doLoadInstForSpaceMiu (LoadInst * LI, 
                            Instruction * sucIns, 
                            DominatorTree & dt, 
                            AAResults & AA,
                            ...
                            Module &M)
{
  ...
  //  assert(ptrTy!=nullptr);
    ...
    if( !bitcastop
        ...
        unsigned destTsize= 
            SolitaryFramer::getBitwidth(desti->getElementType())/8; 
...
void 
SolitaryFramer::doLoadInst (LoadInst * LI, 
                            Instruction * successorInst, 
                            DominatorTree & dt, 
                            AAResults & AA,
                            Module &M)
    ...
    if (issafeaccess==SAFESTATICALLY) { 
      ...
      // compare memaccess size and heap objsize.  
        ...
        unsigned typeSizeOfLoadedContent= 
            SolitaryFramer::getBitwidth(LI->getType())/8;
...
Value* SolitaryFramer::tagFramerTyID (Value * val, //val: alloca or gv
                              unsigned id, 
                              Module &M,
...
/*bool SolitaryFramer::isTaggedPointer (Value* ptrop)
{
    bool istagged=false;
...
void
SolitaryFramer::doStoreInstForSpaceMiu (StoreInst * SI, 
                            Instruction * sucIns, 
                            DominatorTree & dt, 
                            AAResults & AA,
                            vector <AllocaInst*> & ais,
...
void SolitaryFramer::doStoreInst (StoreInst * SI, 
                          Instruction * successorInst, 
                          DominatorTree & dt, 
                          Module &M)
{
    ...
    unsigned srcFramerTySize= SolitaryFramer::getBitwidth(srcType)/8;
...
void SolitaryFramer::doGetElementPtrInst (GetElementPtrInst * GEP, 
                                        Instruction * successorInst, 
                                        Module &M)
{
...
void SolitaryFramer::doFPToSIInst (FPToSIInst * FTS, 
                                Instruction * successorInst, 
                                Module &M)
{
    ...
    Value * numBytesOfSrc     = 
        constructValueFromFunc(SolitaryFramer::getBitwidth(SrcTy)/8, 
                                hook, 
                                0);
    Value * numBytesOfDest    = 
        constructValueFromFunc(SolitaryFramer::getBitwidth(DestTy)/8, 
                                hook, 
                                1);
...
void SolitaryFramer::doSIToFPInst   (SIToFPInst * SFI, Instruction * successorInst, Module &M)
{
    ...
    Value * numBytesOfSrc     = constructValueFromFunc (SolitaryFramer::getBitwidth(SrcTy)/8, hook, 0);
    Value * numBytesOfDest    = constructValueFromFunc (SolitaryFramer::getBitwidth(DestTy)/8, hook, 1);
...
void SolitaryFramer::doFPExt (FPExtInst * FEI, Instruction * successorInst, Module & M)
{
    ...
    unsigned numBytesOfSrcTy = SolitaryFramer::getBitwidth(SrcTy)/8;
    unsigned numBytesOfDestTy = SolitaryFramer::getBitwidth(DestTy)/8;
...
void SolitaryFramer::doPtrToIntInst (PtrToIntInst * PTI, Instruction * successorInst, Module &M)
{
...
void SolitaryFramer::doIntToPtrInst (IntToPtrInst * ITP, Instruction * successorInst, Module &M) 
{
    /*
...
void SolitaryFramer::doAddrSpaceCastInst (AddrSpaceCastInst * I, Instruction * successorInst, Module & M)
{
...
/* not used
Value * SolitaryFramer::castingArgToParam (Instruction * I, Type * paramType, Value * arg)
{
...
*/
Instruction* 
SolitaryFramer::createCastInst (Value * val, 
...
/*
void SolitaryFramer::pushtoarglist (Instruction * insertBeforeMe,  
                            Type * paramTy,
                            Value * arg, 
                            vector<Value*> & arglist,
                            Module & M) 
{
    if (paramTy != arg->getType()) {
        ...
        if (paramTy->isPointerTy() && arg->getType()->isPointerTy()){
            ...
            if (paramTy->getIntegerBitWidth()!=SolitaryFramer::getBitwidth(arg->getType())) {
                BitCastInst * mybitcastinst= new BitCastInst(arg, Type::getIntNTy(M.getContext(),SolitaryFramer::getBitwidth(arg->getType())), "", insertBeforeMe);
...
void SolitaryFramer::doTruncInst (TruncInst * TR, Instruction * successorInst, Module &M)
{
    Function * hook = M.getFunction ("FRAMER_forall_trunc");
    Type * SrcTy    = TR->getSrcTy(); 
    Type * DestTy   = TR->getDestTy();

    assert (SrcTy->isIntegerTy() && DestTy->isIntegerTy() && (SolitaryFramer::getBitwidth(SrcTy) > SolitaryFramer::getBitwidth(DestTy))
    ...
    Value * numBytesOfSrcTy= constructValueFromFunc(SolitaryFramer::getBitwidth(SrcTy)/8,hook,1); 
    Value * numBytesOfDestTy=constructValueFromFunc(SolitaryFramer::getBitwidth(DestTy)/8,hook,2);
...
void SolitaryFramer::doSExtInst (SExtInst * SI, Instruction * successorInst,  Module &M)
{
...
void SolitaryFramer::doZExtInst (ZExtInst * ZI, Instruction * successorInst, Module &M)
{
...
/*
void SolitaryFramer::doOverflowBinaryOp (OverflowingBinaryOperator * OFO, Instruction * successorInst, Module & M)
{
...
void SolitaryFramer::doBinaryOp (BinaryOperator * OFO, Instruction * successorInst, Module & M)
{
    doAddOp (OFO, successorInst, M);
}

void SolitaryFramer::doPossiblyExactOp  (PossiblyExactOperator * PEO, Instruction * successorInst, Module & M)
{
...
void SolitaryFramer::doSubOp (SubOperator * SO, Instruction * successorInst, Module &M)
{
   ...
   //TODO: if it is NUW or NSW..?? wtf is that?
    ...
    Value * numBytesOfIntType= constructValueFromFunc (SolitaryFramer::getBitwidth(OpTy)/8, hook, 2);
...
void SolitaryFramer::doMulOp (MulOperator * MO, Instruction * successorInst,  Module &M)
{
...
//void SolitaryFramer::doAddOp (AddOperator * AO, Instruction * successorInst, Module &M)
void SolitaryFramer::doAddOp (BinaryOperator * OFO, Instruction * successorInst, Module &M)
{
    // perlbench
    if (SolitaryFramer::getBitwidth(OFO->getType())>64) {
        return; 
    }
    //perlbench
...
void SolitaryFramer::doSDivOp (SDivOperator * SDO, Instruction * successorInst, Module & M)
{
...
void SolitaryFramer::doUDivOp (UDivOperator * UDO, Instruction * successorInst, Module &M)
{
...
void SolitaryFramer::doLShrOp (LShrOperator * UDO, Instruction * successorInst, Module &M)
{
...
void SolitaryFramer::doAShrOp (AShrOperator * OP, Instruction * successorInst, Module &M)
{
...
void SolitaryFramer::insertFunctionCall(Function * F, 
...
CallInst * SolitaryFramer::insertHookAndReturn(Function * F, 
                                            Instruction * insertBeforeMe, 
                                            vector<Value*> & arglist) 
{
    CallInst * CI = CallInst::Create (F, arglist, "");
...
/* not used
Value * SolitaryFramer::getEndAddrforMalloc (Value * baseAddr, Value * numBytes, Type * typeToConvertTo, Instruction * insertBeforeMe, Module & M)
{
    IntegerType * my64IntTy = Type::getInt64Ty (M.getContext());
...
Type* SolitaryFramer::getResultTypeOfMalloc (CallInst * CI)
{
...
Instruction* SolitaryFramer::doCallInstMalloc (CallInst * CI, 
                                            Instruction * successorInst, 
                                            Module & M)
{
...
Instruction* SolitaryFramer::doCallInstFree(CallInst * CI, 
                                        Instruction * successorInst, 
                                        Module & M)
{
...
void 
SolitaryFramer::doCallLLVMMemTransfer (CallInst * CI,
                                Instruction * sucIns, 
                                AAResults & AA,
                                vector <AllocaInst*> & ais,
                                Module &M)
{
  ...
  vector<Value *> src_arglist;
    ...
    Function * hook_untag= M.getFunction("FRAMER_untag_ptr");
...
void 
SolitaryFramer::doCallLLVMMemSet (CallInst * MSI, 
                                  Instruction * successorInst, 
                                  Module &M)
{
    /* (Src, Dest, numBytesToCopy, alignment, isVolatile )*/    
    ...
    if (MemSetInst * MI = dyn_cast<MemSetInst>(MSI)){
        destAddress     = MSI->getRawDest();
        val             = MSI->getValue();
        numBytesToCopy  = MSI->getLength();
        numAlign        = MSI->getAlignmentCst();
        isVolatile      = MSI->getVolatileCst(); 
    }
    else {
...
//perlbench
void SolitaryFramer::doCallstrlentemp(
                        CallInst *CI,
                        Instruction *successorInst, 
                        Module &M) {
...
//perlbench
void SolitaryFramer::doCallstrchr(CallInst *CI,Instruction *successorInst, Module &M) {
...
void SolitaryFramer::doCallstrcpy(CallInst * CI, Instruction * successorInst, Module &M) 
{
    Function * hook = M.getFunction ("FRAMER_forall_call_strcpy");
    Function * hooksrc = M.getFunction ("FRAMER_untag_ptr");
...
void SolitaryFramer::doCallstrn___ (CallInst * CI, Instruction * successorInst, Module &M, unsigned temp) 
{
...
void SolitaryFramer::handleArgAttr (CallInst * CI, Module & M)
{
    Function * hook=  M.getFunction ("FRAMER_untag_ptr");
    assert(isa<Function>(CI->getCalledValue()) && 
            "handleArgAttr's calledval is not function! do something..\n");
    
    Function * calledf= cast<Function>(CI->getCalledValue());
...
void SolitaryFramer::doCallmem___ (CallInst * CI, Instruction * successorInst, Module &M) 
{
    Function * hook = M.getFunction ("FRAMER_forall_call_strn___");
...
void SolitaryFramer::doCallExternalFunc (CallInst * CI, Instruction * successorInst, Module &M)
{
...
Instruction* 
SolitaryFramer::doCallInst (CallInst * CI, 
                            Instruction * successorInst,
                            AAResults & AA,
                            vector <AllocaInst*> & LocalUnions,
                            Module &M)
{
    ...
    if (Function * calledfunc = dyn_cast<Function>(calledValue))  { 
        ...
        if (calledValue == M.getFunction(StringRef("malloc")) ||
            ...
            doCallLLVMMemTransfer (CI, successorInst, AA, LocalUnions, M);
      ...
      //  else if (calledfunc->getName().startswith("BZ2_bzWrite")){  
      //      doCallExternalFunc (CI, successorInst, M);
      //  }
        else {
            ;
...
////////////
        ...
        if (HookDefForCalledFuncExists[LLVMMemTransferIsCalled]) {
            doCallLLVMMemTransfer (MCI, successorInst, AA, LocalUnions, M);
        }
    }    
...
void 
SolitaryFramer::doBitCastInst (BitCastInst * BCI, 
                               Instruction * sucIns,
                               AAResults & AA,
                               vector <AllocaInst*> & ais, 
                               Module & M)
{
    ...
    errs()<<"  Bitcast SRC: "<<*srcptr<<", DEST: "<<*destptr<<"\n";
    ...
    /* if (!isSafelyCastedTo (src->getElementType(), 
                               dest->getElementType(), 
                               AllTypeList, SafeCasts)) {
    ...
    short desttid= getIdx(AllTypeList,
                          destptr->getElementType());
    unsigned destTsize= 
        SolitaryFramer::getBitwidth(destptr->getElementType())/8; 
    ...
    Value * constdestTsize= 
            constructValueFromFunc(destTsize, hook, 5); 
    Value * constisUnionTy= 
            constructValueFromFunc(isunion, hook, 6); 
...
uint64_t  
SolitaryFramer::getArraySizeforGV (Type * allocatedObjTy) 
{
...
bool SolitaryFramer::isToBeTaggedType (Type* t)
{
    bool isToBeTagged = 0;
...
/*
Value* SolitaryFramer::createPads(Value* val, Module &M)
{
...
/*
void SolitaryFramer::padObject (Value* obj, Module &M)
{
    Constant * nullHeader = Constant::getNullValue(HeaderTy);
    Constant * nullPad = Constant::getNullValue(OffByOneTy);
...
Constant* SolitaryFramer::tagConstantPtr (Constant* ptr, 
                                        Constant* tag, 
                                        Constant* flag, 
                                        Module & M)
{
...
Constant* SolitaryFramer::getPaddedEndInt(GlobalVariable* paddedGV, Module & M)
{
...
Constant* SolitaryFramer::getLogFrameSize (Constant* base, Constant* end, Module & M)
{
...
Constant* SolitaryFramer::getOffset (Constant* base, Module & M)
{
    //mask=. flag =1
...
Constant* 
SolitaryFramer::getTaggedConstantPtr( GlobalVariable* basePadded,
                                      Constant* origPtr,
                                      Module & M)
{
...
/*void SolitaryFramer::updateHeaderIfSmalledFramed (Constant* tagged, GlobalVariable* ) 
{
...
Constant* 
SolitaryFramer::createHeaderInitializer (GlobalVariable* GV,
                                         unsigned Tid,
                                         unsigned numElems,
...
GlobalVariable * 
SolitaryFramer::createGVHoldingTag (Constant * origGV, 
...
GlobalVariable* 
SolitaryFramer::doGlobalVariable (GlobalVariable * GV, 
                                  Function * F, 
                                  Instruction * insertBeforeMe, 
                                  Module & M,
                                  ...
                                  set<tuple<GlobalVariable*, Constant*, Constant*>> & inits)
{
    ...
    if (ArrayType * arr = dyn_cast<ArrayType>(assignedObjTy)) {
        ...
        numBytesOfElemTy = (SolitaryFramer::getBitwidth (elemTyOfArray))/8;
        ...
        numElems= 1;
        numBytesOfElemTy= (SolitaryFramer::getBitwidth (assignedObjTy))/8; 
        assert(numBytesOfElemTy!=0 &&"numBytesOfElemTy is zero\n");
...
/*
void SolitaryFramer::doGlobalVariable (GlobalVariable * GV, Function * F, Instruction * insertBeforeMe, Module & M)
{
    ...
    if (numElems == 1) {
        numBytesOfElemTy = (SolitaryFramer::getBitwidth (assignedObjTy))/8; //get argument for getbitwidth. (it should be elemty)
        errs()<<"GV non Array. #bytesofElemTy: "<<numBytesOfElemTy<<"\n";
    }
    else {
        //numBitsOfElemTy = SolitaryFramer::getBitwidth (cast<ArrayType>(GV->getType())->..? //how can I get the type of global array?
        numBytesOfElemTy = (SolitaryFramer::getBitwidth (cast<ArrayType>(assignedObjTy)->getElementType()))/8; //get argument for getbitwidth. (it should be elemty)
...
bool SolitaryFramer::hasConstructorAttribute (GlobalVariable * GV)
{
    return (GV->hasAppendingLinkage() && GV->getName().equals(StringRef("llvm.global_ctors"))); 
}

bool SolitaryFramer::hasDestructorAttribute(GlobalVariable *GV)
{
    return (GV->hasAppendingLinkage() && GV->getName().equals(StringRef("llvm.global_dtors"))); 
}

void SolitaryFramer::runOnGlobalVariables (Module &M)
{
...
void SolitaryFramer::flagHooksDefinitionExists (Module &M)
{
...
void SolitaryFramer::create1stBBOfMain (Module &M)
{
   fstBBOfMainFunction->splitBasicBlock(&*fstBBOfMainFunction->begin(), "fstBBOfMain");
}

void 
SolitaryFramer::doInitFuncHook (Module & M)
{
    Function * F= 
    ...
    errs()<<"type2: "<<*rtOffsetTbl->getType()<<"\n";
    
    Value * numStructTy= 
            constructValueFromFunc (structTyCount, F, 0);
    /*
    Value * castcolnum= 
            constructValueFromFunc(colcount, F, 3);  
    Value * castrownum= 
            constructValueFromFunc((unsigned)SafeCasts.size(), F, 4); 
    */
    Value * consttotalofs= 
    ...
    pushtoarglist (insertBeforeMe,
                   F->getFunctionType()->getParamType(3),
                   castcolnum, arglist, M);
    pushtoarglist (insertBeforeMe,
                   F->getFunctionType()->getParamType(4),
                   castrownum, arglist, M); */
...
void SolitaryFramer::assertBasicTyTable(Module &M)
{
...
/*
void SolitaryFramer::createUserStructTyList (Module &M)
{
...
unsigned SolitaryFramer::getMaxFieldCountOfStruct()
{
    unsigned maxcount=0;
...
void SolitaryFramer::createStructTypeTable(Module &M) 
{
...
void SolitaryFramer::doInitialSetup (Module &M)
{
    ...
    // * now lets save union typed gv lists..
...
bool 
SolitaryFramer::runOnBasicBlock (BasicBlock &BB, 
                                 DominatorTree & dt, 
                                 AAResults & AA,
                                 vector <AllocaInst*> & LocalUnions,
                                 Module &M)
        ...
        if (AllocaInst * AI = dyn_cast<AllocaInst>(it) ){
          ...
          */ 
            ...
            Instruction * toBeRemoved= 
                doCallInst (CI, successorInst, AA, LocalUnions, M);
...
/*
bool SolitaryFramer::isLeafOfDomTree (BasicBlock* bb, PostDominatorTree & postDT)
{
    bool isLeaf=true;
...
*/
void SolitaryFramer::insertEpilogueOfFunc (Instruction* tagged, 
                                        Module &M)//, 
                                        //PostDominatorTree & postDT)
{
...
/*
void SolitaryFramer::insertEpilogueOfFunc (AllocaInst* tagged, Module &M, DominatorTree & FDomTree)
{
...
void SolitaryFramer::doExitingMain (Module &M)
{
    vector <Value*> arglist;
...
bool 
SolitaryFramer::runOnFunction (Function &F, 
                               Module &M)
{
  ...
  /* end of SPACEMIU */
    ...
    for (Function::iterator fi = F.begin(), 
        ...
        SolitaryFramer::runOnBasicBlock (*fi, 
                                         dt, 
                                         aaresult, 
                                         LocalUnions, 
                                         M);//, postDT); 
...
bool SolitaryFramer::runOnModule (Module &M) 
{
    solitaryframerpasscounter++;
...
//  #endif 
    ...
    // so commmented out.
    //createRTSafeCastsTable (SafeCasts, SafecastTable, M); 
