...
struct Framer : public ModulePass {
  public:
    ...
    Framer() : ModulePass(ID) {
        initializeFramerPass(*PassRegistry::getPassRegistry());    
    }
    virtual ~Framer() {}
  ...
  protected:
     ...
     //                                      Constant * untaggedGV);
      ...
      Instruction* doCallInst (CallInst * CI, 
                            ...
                            #endif
                               Module &M);
      Instruction * doCallInstMalloc(CallInst * CI,
                                     Instruction * successorInst,
                                     AAResults & AA,
                                     DominatorTree & DT,
                                     CallGraph & CG,
                                     Module & M);
      Instruction* doCallInstFree (CallInst * CI, 
                                   Instruction * successorInst, 
                                   Module & M);
      Instruction * doCallInstRealloc (CallInst * CI, 
                          Instruction * successorInst, 
                          AAResults & AA,
                          DominatorTree & DT,                          
                          CallGraph & CG,
...
//      bool isSafePtr (Value * p);
      ...
      void doLazyTypeUpdate (Function * wrp, 
                             CallGraph & CG, 
                             DominatorTree & DT, 
                             Module & M);
      void hookWrapperCallSite (Function * caller, // wrapper's caller
                                Function * wrapper,
                                DominatorTree & DT,
                                Module & M);
