...
extern short*  FRAMER_baseOfSafeCastTable;
...
//#endif //end of ifNdef of enable_castcheck 
                    ...
                    ) 
{
  ...
  #else 
       ((uint64_t)MaxOffset+1)*2*(uint64_t)objTyid; 
       //doubled by 2 since it's short-typed (2 bytes).
  #endif
*/
  unsigned idx= (MaxOffset+1)*2*(int)objTyid;
  return  FRAMER_baseOf_OffsetTable+idx;
}
#endif
...
dbg(    printf("\t* baseOf_OffsetTable:\t%p\n",       
  ...
  for (short i=0; i< maxoffsperty; i++) {
    ...
    if (offset > *ptr)  continue;
    else if (offset < *ptr)  return false; 
    
    ptr++; //move to the array of tids
     
    for (short j=0;j< maxtysperoff;j++){ 
      
      short tid= *ptr; // type ID at the offset
...
#else
  //TODO
  // (1) get the tyid of the offset of the object
  // (2) until we meet -1, compare it. 

  short tidatoff= *(initptr + offset); //initptr is short* typed. 
  short * safeentry= FRAMER_baseOfSafeCastTable //short * typed 
                    + max_upcasts * tidatoff; 


dbg(
  printf("\ttid at off:\t%d\n", tidatoff);
  printf("FRAMER_baseOfSafeCastTable ++++\n");
  
  for (short i=0; i< 27; i++) { 
    short * tempptr= FRAMER_baseOfSafeCastTable
                     + i*max_upcasts;
    for (short j=0; j< max_upcasts; j++) {
        printf(" %d ", *(tempptr+j));
    }
    printf (" ** \n");
  }
  ...
  for (short i=0; i<max_upcasts; i++) {

    short temptid= *(safeentry+i);
                    ...
                    )
{
  ...
  dbg(  
    ...
    if (FRAMER_is_heap_array(mptr)) {
        dbg(printf("\t--> heap_array. returning\n");)
    ...
    /* if it's allocated by malloc family, 
        and casted to its effective type.
        Its first bitcast after malloc. */

/// DEL if macro works.
    
    if (mptr->FramerTyID == FRAMER_is_heap_obj){ 
      
      FRAMER_handle_heap_object_for_cast(
            untagged_p, (uint64_t)base, objsize, destyid,
            destTsize, mptr, isUnionTy);
      
      dbg(printf("\t--> heap object. update tid and returning. ID: %d\n", mptr->FramerTyID);)
...
*/
   ...
   // unsigned elemsize= mptr->elemsize;
