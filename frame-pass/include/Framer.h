...
struct Framer : public ModulePass {
  public:
    ...
    Framer() : ModulePass(ID) {
        //llvm::initializeFramerPass(*PassRegistry::getPassRegistry());    
    }
    //virtual ~Framer() {}

    virtual bool runOnModule(Module &M);
    virtual bool runOnFunction (Function &F, Module &M); 
    virtual bool runOnBasicBlock (BasicBlock &BB, DominatorTree & dt, Module &M);
  ...
  protected:
     ...
     // bool isPrintfStr             (GlobalVariable * GV);
      ...
      Instruction* castAndreplaceAllUsesWith (Value * originalPtr, 
              Value * modifiedPtr);
      GlobalVariable * 
createGVHoldingTag (Constant * origGV, Constant * tagged, Module & M,GlobalVariable * paddedGV, GlobalVariable * GVToDel);
      ...
      Constant * getConstExprReplacement (Use* use, Constant* newGV, GlobalVariable * origGV);
     ...
     //                                      Constant * untaggedGV);
      ...
      Instruction* doArrayAllocaInst      (AllocaInst * AI, 
              ...
              Module & M);
      //PostDominatorTree & postDT);
      void doLoadInst             (LoadInst * LI, Instruction * successorInst, DominatorTree & dt, Module & M);
      void doStoreInst            (StoreInst * SI, Instruction * successorInst, DominatorTree & dt, Module &M);
