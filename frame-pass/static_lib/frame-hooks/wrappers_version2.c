...
extern void* FRAMER_untag_ptr (void * p);
...
void *__wrap_malloc(size_t allocatedSizeInBytes)
{
  ...
  #ifdef ENABLE_TYPECAST_CHECK    
    FRAMER_update_header(p, FRAMER_is_heap_obj, raw_obj_size, raw_obj_size);
  #else    
...
void __wrap_free (void* obj_base)
{
    ...
    //    FRAMER_untag_ptr_int((uintptr_t)obj_base);
        (uintptr_t)FRAMER_untag_ptr(obj_base);
...
void *__wrap_realloc(void* thin_base, size_t size)
{
    ...
    assert_dbg(assert(tagvec!=0 && " Not tagged.");)
    assert(tagvec!=0);
    dbg(if (tagvec==0) {
    ...
    uintptr_t untagged_p= 
        ...
        (uintptr_t)FRAMER_untag_ptr(thin_base);
  ...
  #ifdef ENABLE_TYPECAST_CHECK
    FRAMER_update_header(p, FRAMER_is_heap_obj, size, size);
  #else
...
void *__wrap_calloc(size_t elems, size_t elemsize)
{
  ...
  #ifdef ENABLE_TYPECAST_CHECK
    FRAMER_update_header(p, FRAMER_is_heap_obj, raw_obj_size, elemsize);
  #else
    FRAMER_update_header(p, VoidTyID, raw_obj_size);
  #endif
...
char *__wrap_strcpy(char * dest, char * src)
{
    ...
    __real_strcpy((char*)FRAMER_check_mem_access(dest, srclen), untagsrc);
    return dest; 
}
...
{
    return __real_memcmp(FRAMER_check_mem_access(str1, n), 
                        FRAMER_check_mem_access(str2, n),n);
}
