...
Instruction * Framer::getNextInst (Instruction * Instr)
{
                                            ...
                                            //PostDominatorTree & postDT)
{
    ...
    if (notDynamicArray){ 
        ...
        //vector<Type*> paddedObjTyList= {HeaderTy, AI->getAllocatedType(), OffByOneTy}; //** offbyone
        ...
        //Constant * nullPad = Constant::getNullValue(OffByOneTy); //** offbyone. deleted.
                 ...
                 Type * typeToConvertTo, Instruction * insertBeforeMe, Module & M)
{
    ...
    dbg(errs()<<"numBytesOfElem:\t"<<*numByteOfElem<<"\n";)
    ...
    dbg(errs()<<"totalNumBytes:\t"<<*totalNumBytes<<"\n";)
...
uint64_t Framer::getBitwidthOfStruct ( StructType * STy )
{
    ...
    if ( STy->isPacked()) { //no padding
        for (unsigned i=0;i<STy->getNumElements();++i) {
            sumBits = sumBits + Framer::getBitwidth (STy->getElementType(i), false);
        }
        return sumBits;
    }
...
unsigned Framer::getBitwidth (Type * allocatedType, bool isOutMost)
    ...
    array->#bits of array element. (outmost element, if the array is array of array...)*/
{
    ...
    if (( allocatedTyID > Type::VoidTyID && allocatedTyID < Type::PPC_FP128TyID) ){
        ...
        if (isa<PointerType>(allocatedType)) {
            ...
            if (isOutMost) {
                return Framer::getBitwidth(arrayTy->getElementType(), false);
            }
            else {
                return (unsigned)(arrayTy->getNumElements())*Framer::getBitwidth(arrayTy->getElementType(), false); 
            }
        }
        else if (VectorType * vecty= dyn_cast<VectorType>(allocatedType)) {
            if (vecty->getBitWidth()!=0) {
                //errs()<<"VECTsize<8::  "<<*vecty<<"\n";
                return vecty->getBitWidth();
            }
            else {
                return (unsigned)(vecty->getNumElements())*Framer::getBitwidth(vecty->getElementType(), false); 
            }
        }
        else {
                                               ...
                                               Value* modifiedPtr)
{
    ...
    /*else if (originalPtr->getType()==modifiedPtr->getType() 
        && isa<Constant>(originalPtr)){ //old=new type && constant.
        ...
        while ( nextUse ) {
            ...
            if (Instruction * instUser = dyn_cast<Instruction>(user)){
                ...
                ///// perlbench
                if (originalPtr->getName().equals("PL_colors")) {        
                    if (instUser->isCast()) { 
                        errs()<<"PL_colors bitcast is instruction!!! check\n";
                        errs()<<*instUser<<"\n";
                        exit(0);
                    }
                }
                ...
                if (originalPtr->getName().equals("PL_colors")) {        
                    if (constUser->isCast()) {
                        errs()<<"BEFORE constexpr user: "<<*constUser<<"\n"; 
                         
                            ////constUser->getWithOperandReplaced(0,cast<Constant>(modifiedPtr));   
                            ////errs()<<"newuser: "<<*newuser<<"\n";  
                    }
                }
                ///////////
                
                // constUser->handleOperandChange(*currentUse, modifiedPtr);
                (&*currentUse)->set(modifiedPtr); 
                ...
                if (originalPtr->getName().equals("PL_colors")) {        
                    if (constUser->isCast()) {
                        ...
                        errs()<<"AFTER constexpr user: "<<*constUser<<"\n"; 
                    }
                }
                ///////////
            ...
            else if (GlobalVariable * gvuser= dyn_cast<GlobalVariable>(user)) {

                /// perlbench
                if (originalPtr->getName().equals("PL_colors")) {        
                    errs()<<"global variable"<<*constUser<<"\n"; 
                }
                ///////////
                
                ///- handleOperandChange does NOT support GV. -///
                assert(isa<Constant>(modifiedPtr)
                        && "FRAMER: originalPtr is constant, but modifiedPtr(padded) is not?");
                //gvuser->setInitializer(cast<Constant>(modifiedPtr));
                (&*currentUse)->set(modifiedPtr);
            }
            else if (isa<ConstantAggregate>(user) 
                    || isa<ConstantData>(user)) {
                
                Constant * constuser= dyn_cast<Constant>(user); 
                
                /// perlbench
                if (originalPtr->getName().equals("PL_colors")) {        
                    if (isa<ConstantAggregate>(user)){errs()<<"ConstantAggregate\n";}
                    if (isa<ConstantData>(user)){errs()<<"ConstantData\n";}
                }
               ///////////
               
               //constuser->handleOperandChange(*currentUse, modifiedPtr);
               (&*currentUse)->set(modifiedPtr); //--> do this??
            }
            else {
                dbg(errs()<<"user:: "<<*user<<"\n";)
...
unsigned Framer::getopidx (User *user, Use * op)
{
...
ConstantAggregate* Framer::getNewConstAggr(
    int opnum, Constant * CA, Constant * newGV) 
{
    ...
    //errs()<<"\topnum: "<<opnum<<"\n";
    //errs()<<"\torigCA: "<<*CA<<"\n";
    //errs()<<"\treplica: "<<*replica<<"\n";
    return cast<ConstantAggregate>(replica);
}

/// CU == constant_expr or aggr (orig_gv) form.
Constant* Framer::getNewConstExpr (Use * use, Constant* newGV, 
...
{      // use: bitcast (oldgv to ty) 
    ...
    for (User::op_iterator it= CU->op_begin(), ie=CU->op_end(); it!=ie; ++it) { 
        ...
        // perlbench 
        //if (origGV->getName().equals("PL_sv_undef")) {
        //    errs()<<" "<<opnum<<"/"<<CU->getNumOperands()<<". op: "<<**it; 
        //}
        // perlbench
         
        if ((&*it)->get()==origGV) {
            if (ConstantExpr * COP= dyn_cast<ConstantExpr>(CU)) {
                replica= cast<ConstantExpr>(replica)->getWithOperandReplaced(opnum, newGV); 
            }
            else if (ConstantAggregate * COP= dyn_cast<ConstantAggregate>(CU)) {
                //errs()<<" HERE. replacing constantvector elem.\n";
                //replica->handleOperandChange(origGV, newGV); 
                //errs()<<"replacing origGV\n"; 
            ...
            //perlbench
        //    if (origGV->getName().equals("PL_sv_undef")) {
        //        errs()<<"new op(expr or aggr) pushedback: "<<*itex<<"\n"; 
        //    }
            //perlbench
            ...
            if (isa<ConstantExpr>(CU)) {
                ...
                //replica->handleOperandChange(itex,temp); 
                //errs()<<"replacing subtree\n"; 
...
Constant * Framer::getReplacement (Use* use, Constant* newGV, GlobalVariable * origGV)
...
void Framer::handleUsesOfGV(Use* use, Constant* tagged, 
    Constant* untagged, GlobalVariable * origGV)
{
    ...
    if(GlobalVariable * GVU= dyn_cast<GlobalVariable>(user)) {  
        ...
        GVU->setInitializer(replica);
           // perlbench 
        if (origGV->getName().equals("PL_savestack")) {
            errs()<<" GV initializer USER. New: "<<*GVU<<"\n";  
            exit(0);
        }
      // perlbench
      ...
      // perlbench 
        if (origGV->getName().equals("PL_savestack")) {
            errs()<<" INST USER\n";  
            if (isa<ConstantVector>((&*use))) {
                ConstantVector * vec= cast<ConstantVector>((&*use)->get());
                errs()<<"op 0: "<<*vec->getOperand(0)<<"\n";
                errs()<<"op 1: "<<*vec->getOperand(1)<<"\n";
            }   
        }
        // perlbench
       
        if (!isHookFamilyFunction(IU->getParent()->getParent()) && IU->getParent()) { 
            // ** TODO: check..
            Constant* replica= getReplacement(use,tagged, origGV);
            ...
            errs()<<"what case is this? handleusesofgv.\n"; 
            exit(0);
        }
    }
        ...
        while ( nextUse ) {
            ...
            handleUsesOfGV(currentUse,tagged,untagged,origGV); //TODO: check if it's user or cit 
            usenum++;
...
void Framer::castAndreplaceAllUsesWithForGV (GlobalVariable * originalGV, 
        Constant * taggedGV, Constant * untaggedGV)
{
    ////// perlbench
    if (originalGV->getName().equals("PL_savestack")) {
          errs()<<"PL_savestack castAndreplaceAllUsesWithForGV\n";
          int tempnum=0;
          ...
          while ( nextUse ) {
              nextUse = currentUse->getNext();
              errs()<<tempnum<<": "<<*currentUse->getUser()<<"\n";
              currentUse = nextUse;
              tempnum++;
          }
    }
    ...
    /// perlbench
    if (originalGV->getName().equals("PL_savestack")) {
        errs()<<"PL_savestack getnumuse::: "<<originalGV->getNumUses()<<"\n";
    }
    ////perlbench
    ...
    while ( nextUse ) {
        ...
        handleUsesOfGV(currentUse, taggedGV, untaggedGV, originalGV); 
...
bool Framer::isHookedAllocaOrGV(Value * p) 
{
    //alloca
    if (isa<CallInst>(p->stripPointerCasts())) { 
        ...
        if (isa<Function>(CV)) {
            ...
            if (F->getName().startswith("FRAMER_forall_alloca")) { //for alloca
                //errs()<<"HOOKED alloca(skip hook) "<<*CI<<"\n";
        ...
        if(it != paddedGVs.end()) {
            //errs()<<"HOOKED GV (skip hook): "<<*itp->getOperand(0)<<"\n";
            return true;
        }
    }
    return false;
...
unsigned Framer::getStaticOffset (GEPOperator * gep)
{
    assert (isa<CompositeType>(cast<PointerType>(gep->getPointerOperandType())->getElementType()) 
    ...
    for (unsigned i=0;i<gep->getNumIndices();i++){
        ...
        //errs()<<" idx: "<<idx<<"\n"; 
        if (i==0){
            assert(idx==0 && "idx is not 0. do something.\n");
            continue;
        }
        ...
        if (SequentialType * SEQT=dyn_cast<SequentialType>(upT)){
            ...
            //errs()<<"   SType size: "<<SL->getElementOffset(idx)<<"\n";
...
Value* Framer::getAssignment(LoadInst * LI, DominatorTree & dt)
{
...
bool Framer::checkSafeWithDomTree (Value * op, DominatorTree & dt) 
{
    ...
    //errs()<<"DOM CHECK.LI SI's strip ptrOP: "<<*op<<"\n";
    if (LoadInst * li= dyn_cast<LoadInst>(op)) { //li == %1 == op
        //errs()<<"  ptr op: "<<*li->getPointerOperand()<<"\n"; 
        ...
        if (isSafePtr(itsval)) {
            //errs()<<"SAFE(Dom) :"<<*itsval<<"\n";
            return true;      
        }
        else {
            //errs()<<"got assignment: "<<*itsval<<"\n";
            // if (double pointers or triple pointers? anyway) 
            // then return true? since we tag typeID only for alloca or global referencing  
            if (isa<LoadInst>(itsval)) {
                LoadInst * pp= cast<LoadInst>(itsval);
                //errs()<<"load's op: "<<*pp->getPointerOperand()<<"\n";
            }
            exit(0);
        }   
        //  }
        ...
        for (int i=0;i<pn->getNumIncomingValues();i++) {
            errs()<<"\tphi op "<<i<<": "<<*pn->getIncomingValue(i)<<"\n"; 
        }
    }
    else {
        //errs()<<"\telse\n";
...
{
    PointerType * pty= cast<PointerType>(ptrop->getType()); 
...
bool Framer::isSafeAccess(Value * op_, DominatorTree & dt) //op: load/store's ptr op
{
    Value * op=op_->stripPointerCasts();

    if (isHookedAllocaOrGV(op)) {
        return true;
    }
    else if (GEPOperator * gep=dyn_cast<GEPOperator>(op)) {  
        if (!gep->hasAllConstantIndices()){
            return false;;  
        }
        if (!isHookedAllocaOrGV(gep->getPointerOperand())) {
            if (isa<LoadInst>(gep->getPointerOperand()->stripPointerCasts())) {
                errs()<<"GEP op not GVorALLOCA: "<<*gep->getPointerOperand()->stripPointerCasts()<<"\n";
            }
            return false; //might be safe, but not handling that now. do something later.
        }
        if (gep->hasAllZeroIndices()) { // base addr of alloca/gv
            return true; 
        }

        unsigned offset= getStaticOffset(gep); // offset= base~ptr (two args. 2nd is gep's ptr.assignment)
        unsigned totalsize= getTotalSize(gep); 
        return isStaticInBound(offset, Framer::getBitwidth(cast<PointerType>(op_->getType())->getElementType()), totalsize); 
        
        // now LI/SI's PtrOp is GEP having constant indices only, 
        // alloca/GV as an ptrop,   
    }
    else {
...
void Framer::doLoadInst (LoadInst * LI, Instruction * successorInst, DominatorTree & dt, Module &M)
{
    ...
    if (isSafeAccess(LI->getPointerOperand(), dt)) { 
    //    if (LI->getFunction()->getName().equals("FindAnagram")|| 
    //            LI->getFunction()->getName().equals("SortCandidates")) {
    //        errs()<<"SAFE L: "<<*LI->getPointerOperand()<<"\n";
    //    }
       // errs()<<"     op: "<<*LI->getPointerOperand()<<"\n";
       // errs()<<"   strp: "<<*LI->getPointerOperand()->stripPointerCasts()<<"\n";
        tempsafe++;
        ...
        if (LI->getFunction()->getName().equals("FindAnagram")|| 
                LI->getFunction()->getName().equals("SortCandidates")) {
            errs()<<"UNSAFE L: "<<*LI->getPointerOperand()<<"\n";
        }
        hook= M.getFunction ("FRAMER_forall_load");
        
        unsigned FramerTyID= getFramerTypeID(LI->getType());
        ...
        Value * typeIDOfLoadedContent= 
            constructValueFromFunc (FramerTyID, 
                                    hook, 
                                    1);
                          ...
                          Module &M)
{
    ...
    if (isSafePtr(SI->getPointerOperand())) {
        //errs()<<"NoTg S: "<<*SI<<"\n";
        //errs()<<"    op: "<<*SI->getPointerOperand()<<"\n";
        safeptr++;
        return; 
    }
    if (isSafeAccess(SI->getPointerOperand(), dt)) {
      //  if (SI->getFunction()->getName().equals("FindAnagram")|| 
      //          SI->getFunction()->getName().equals("SortCandidates")) {
      //      errs()<<"SAFE S: "<<*SI->getPointerOperand()<<"\n";
      //  }
        //errs()<<"    op: "<<*SI->getPointerOperand()<<"\n";
        tempsafe++;
        hook= M.getFunction ("FRAMER_supplementary_untag_ptr");
        ...
        if (SI->getFunction()->getName().equals("FindAnagram")|| 
                SI->getFunction()->getName().equals("SortCandidates")) {
            errs()<<"UNSAFE S: "<<*SI->getPointerOperand()<<"\n";
        }
                                        ...
                                        Module &M)
{
    ...
    unsigned resultFramerTyID= 
     ...
     ////perlbench
    if (resultFramerTyID==22) {
        errs()<<"gep. result type: i8>>> "<<*GEP<<"\n";
    }
    ////
   ...
   */
   ...
   /* Value * typeIDOfElem        = 
        constructValueFromFunc(getFramerTypeID(GEPElemType), //GEPElemType->getTypeID(),   
                                ...
                                2);
    Value * NumBytesOfElemTy    = 
        constructValueFromFunc(Framer::getBitwidth(GEPElemType)/8,
                                hook, 
                                3); 
...
/* not used
Value * Framer::castingArgToParam (Instruction * I, Type * paramType, Value * arg)
{
    ...
    if ( paramTy != arg->getType()) {
        ...
        if (paramTy->isPointerTy() && arg->getType()->isPointerTy()){
            ...
            if (paramTy->getIntegerBitWidth()!=Framer::getBitwidth(arg->getType())) {
                errs()<<"paramTy: "<<*paramTy<<", argTy: "<<*arg->getType()<<"\n";
...
void Framer::doAddOp (BinaryOperator * OFO, Instruction * successorInst, Module &M)
{
    ...
    if (Framer::getBitwidth(OFO->getType())>64) {
        errs()<<"doAddOp- bitsize is above 64. skipping: "<<*OFO<<"\n";
        return; 
    }
    //perlbench
...
void Framer::doCallExternalFunc (CallInst * CI, Instruction * successorInst, Module &M)
{
    ...
    if (CI->getFunction()->getName().equals("Perl_pp_gmtime")) {
        errs()<<"   new CI: "<<*CI<<"\n";
    }
    dbg(errs()<<"new CI: "<<*CI<<"\n";)
}
        ...
        if (isa<PHINode>(inst)) {
            ...
            while (!isa<Function>(getfunc->stripPointerCasts())) {
                if (LoadInst * lii = dyn_cast<LoadInst>(getfunc->stripPointerCasts())) {
                    ...
                    errs()<<"else: "<<*(getfunc->stripPointerCasts())<<"\n";
                    exit(0);
                }
            }
            Function * calledfunc= cast<Function>(getfunc);
...
/* not used
Value * Framer::getEndAddressGV (GlobalVariable * GV, uint64_t numElems, unsigned numBitsOfElemTy, 
                                       Type * typeToConvertTo ,Instruction * insertBeforeMe, Module & M)
{
    IntegerType * my64IntTy = Type::getInt64Ty (M.getContext());
...
/*
size_t Framer::pow2roundup (size_t x)
{
...
size_t Framer::getNewAlignment (size_t totalsize)
{
                                    ...
                                    Module & M)
{
 ...
 //////// perlbench ///////////////// 
    ...
    ////////////////////
    // instead of iterating uses of GV, and selectively replacing,
    // trying replacing first, then creating CI, and inserting it. --> removed!
    // GV->replaceAllUsesWith(taggedConstantPtr);

    // now iterating uses of GV again, since an GV initializer with constant expr
    // is illegal.
    castAndreplaceAllUsesWithForGV(GV, taggedConstantPtr, origObj);
...
void Framer::runOnGlobalVariables (Module &M)
{
    ...
    */
    ...
    for (Module::global_iterator gi = M.global_begin(), ge=M.global_end(); gi!=ge ; ++gi) {
        ...
        //perlbench
        if(((&*gi)->getName()).equals("PL_tokenbuf")
            || (&*gi)->getName().equals("PL_savestack")){
            errs()<<"GV: "<<*(&*gi)<<"\n"; 
            if ((&*gi)->hasInitializer()) {
                errs()<<"  initializer: "<<*(&*gi)->getInitializer()<<"\n"; 
            }
        }
        ///perlbench
...
void Framer::doInitialSetup (Module &M)
{
   ...
   /* // moved to after runonmodule. 
    createStructTypeTable(M); //------>TODO: move to after runonmodule? 
...
bool Framer::isHookFamilyFunction (Function * F ) 
...
bool Framer::isSafePtr (Value * ptr)
{
    ...
    if (isa<GetElementPtrInst>(p)) { //li's ptrop == gep. return false.
        return false;
    }
        ...
        if (AllocaInst * AI = dyn_cast<AllocaInst>(it) ){
          ...
          */  
            ...
            if (isSafePtr(LI->getPointerOperand())) {
                //errs()<<"NoTg L's op: "<<*LI->getPointerOperand()<<"\n";
                safeptr++;
                continue;
            }
                                        ...
                                        //PostDominatorTree & postDT)
{
    ...
    if (allocaBB== &(F->front()) && F!=Func_main) { //skip main. since we will unmap anyway..
        ...
        //debug...
        errs()<<"insertEpilogueOfFunc problem\n";
        errs()<<"  inst: "<<*tagged<<"\n"; 
        errs()<<"  func: "<<tagged->getFunction()->getName()<<"\n"; 
        //
        CallInst * CI= CallInst::Create(M.getFunction ("FRAMER_reset_entries_for_alloca"), 
                                    arglist, 
                                    "");
        ...
        if (!(stackSaveCount==1 && scopeStart !=nullptr && mySavedStack != nullptr)) {
            errs()<<"** alloca not in entry BB, nor having llvm.lifetime\n";
...
{
    dbg(errs()<<"Function -- "<<F.getName()<<" processing...\n";)
...
bool Framer::runOnModule (Module &M) 
{
    ...
    for (Module::iterator mi=M.begin(), me=M.end(); mi!=me ; ++mi) {
        ...
        //DominatorTree &dt = getAnalysis<DominatorTreeWrapperPass>(*mi).getDomTree();
    ...
    errs()<<"now doinitFuncHook...\n";
    doInitFuncHook(M);
    errs()<<"tempGVcount: "<<tempGVcount<<"\n";
    errs()<<"tempsafe: "<<tempsafe<<"\n";
    errs()<<"tempload: "<<tempload<<"\n";
    errs()<<"tempstore: "<<tempstore<<"\n";
    errs()<<"safeptr: "<<safeptr<<"\n";
    errs()<<"maxcount100temp: "<<maxcount100temp<<"\n";
    errs()<<"leaving Framer........\n"; 
    return false;
}
