...
static unsigned isSafeAccess(Value * op_, Module & M, bool isMemAccess) //op: load/store's ptr op
{
...
/*
        ...
        Value * obj= isHookedAllocaOrGV(gep->getPointerOperand()->stripPointerCasts()); 

        if (obj==nullptr) {
            // tried returning true, but overheads reduced very little. 
            return false; //might be safe, but not handling that now. do something better.
        }

        if (gep->hasAllZeroIndices()) { // base addr of alloca/gv
            return true; 
        }
        if (gep->getOperand(1)!=0){
            return false; //maybe safe, but cant be bothered to implement it. 
        }
        //unsigned offset= getStaticOffset(gep); // offset= base~ptr (two args. 2nd is gep's ptr.assignment)
        unsigned offset= getStaticOffset(gep, obj, dlayout); // offset= base~ptr (two args. 2nd is gep's ptr.assignment)
        unsigned totalsize= getTotalSize(gep, dlayout); 
        //unsigned elemsize= Framer::getBitwidth(cast<PointerType>(op_->getType())->getElementType());
        unsigned elemsize= FramerGetBitwidth(cast<PointerType>(op_->getType())->getElementType(), dlayout);
        return isStaticInBound(offset, elemsize, totalsize); 
...
void Framer::doLoadInst (LoadInst * LI, Instruction * successorInst, DominatorTree & dt, Module &M)
{
    ...
    unsigned issafeaccess= isSafeAccess(LI->getPointerOperand(), M, 1); 
                          ...
                          Module &M)
{
    ...
    unsigned issafeaccess= isSafeAccess(SI->getPointerOperand(), M, 1);
    if (issafeaccess==SAFESTATICALLY) {
        tempsafe++;
        hook= M.getFunction ("FRAMER_supplementary_untag_ptr");
        ...
        if (SI->getFunction()->getName().equals("FindAnagram")|| 
            ...
            dbg(errs()<<"UNSAFE S: "<<*SI->getPointerOperand()<<"\n";)
        }
    ...
    for (BasicBlock::iterator it = BB.begin(), ie = BB.end();it != ie; ++it) {
        ...
        if (AllocaInst * AI = dyn_cast<AllocaInst>(it) ){
          ...
          */  
            ...
            unsigned issafeaccess=isSafeAccess(GEP, M, 0);
            if (issafeaccess==SAFESTATICALLY) {
                tempsafe++;
                continue; 
            }
...
//perlbench.s.
//if (tagged->getFunction()->getName().equals("retrieve_hook")) {
//    errs()<<"  TI: "<<*TI<<"\n"; 
//}
//perlbench.e 
...
//                assert(isa<CallInst>(TI) && "not call exit? what case is this?\n");
                CallInst * CI= CallInst::Create(M.getFunction ("FRAMER_reset_entries_for_alloca"), 
                        arglist, 
                        "");
//perlbench.s.
...
/////// printing inst
/*errs()<<">>>> Print findanagram 1\n";
if (F.getName().equals("FindAnagram")) {  
    for (Function::iterator fi=F.begin(),fe=F.end();fi!=fe;++fi ){
        for (BasicBlock::iterator it=(&*fi)->begin(),end=(&*fi)->end();it!=end;++it) {
            errs()<<"I1: "<<*it<<"\n"; 
        }
    }
    errs()<<"=====1\n";
